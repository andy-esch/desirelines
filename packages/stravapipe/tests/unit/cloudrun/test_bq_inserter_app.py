...
def client(mock_bq_config):
    ...
    with (
        ...
        # Default: experiment off. Tests that exercise the dual-write
        # path patch this to a MagicMock at the module path.
        patch(
            "stravapipe.cloudrun.bq_inserter_app.make_storage_writer",
            return_value=None,
        ),
    ):
...
class TestExperimentalDualWrite:
    """Coordination tests for the BQ Storage Write API dual-write spike.

    Invariants the experiment depends on:
      1. Experimental write runs ONLY after the production write succeeds.
         (A production failure must short-circuit before any experimental work.)
      2. Errors from the experimental writer NEVER propagate. The handler
         still returns 'created' on response — only the production write
         determines status.
      3. When the feature flag is off (`experimental_writer is None`),
         the experimental code path is fully bypassed.
    """

    def test_experimental_write_runs_when_enabled(self, client):
        from stravapipe.cloudrun.bq_inserter_app import app
        ...
        app.state.writer = mock_writer
        app.state.experimental_writer = mock_experimental
        ...
        mock_writer.write_activity.assert_called_once_with(mock_activity)
        mock_experimental.write_activity.assert_called_once_with(mock_activity)

    def test_experimental_write_failure_does_not_propagate(self, client):
        """Experimental write raises → response is still 'created' (200)."""
        from stravapipe.cloudrun.bq_inserter_app import app

        mock_writer = MagicMock()
        mock_writer.write_activity.return_value = {"rows_affected": 1}
        mock_experimental = MagicMock()
        mock_experimental.write_activity.side_effect = RuntimeError(
            "simulated Storage Write API gRPC error"
        )
        ...
        # The whole point: experimental failure does not affect the response.
        assert response.status_code == 200
        assert response.json()["status"] == "created"
        mock_experimental.write_activity.assert_called_once()

    def test_experimental_write_is_not_attempted_when_disabled(self, client):
        """When experimental_writer is None, no experimental code path runs."""
        from stravapipe.cloudrun.bq_inserter_app import app
        ...
        app.state.writer = mock_writer
        app.state.experimental_writer = None  # feature flag off
        ...
        assert response.status_code == 200
        assert response.json()["status"] == "created"
        # No exception, no extra side effects — production path is unchanged.

    def test_experimental_write_is_skipped_when_production_write_fails(self, client):
        """If production write raises, experimental write must not run.

        The order matters: a production failure short-circuits before we
        even look at the experimental path. This prevents the experiment
        from masking real ingestion failures (and from racing with the
        retry that Pub/Sub will trigger).
        """
        from stravapipe.cloudrun.bq_inserter_app import app
        ...
        # Production failure surfaces as 500
        assert response.status_code == 500
        # Experimental writer never invoked
        mock_experimental.write_activity.assert_not_called()
