...
// GetRouteRings returns distance rings centered at (0,0) in the same coordinate space
// as normalized routes. Uses the average latitude of activity start points with PostGIS
// geography type for accurate degree-to-meter conversion regardless of latitude.
func (r *ActivityRepository) GetRouteRings(ctx context.Context, userID string, ringMeters []int) (rings []repository.RouteRing, retErr error) {
	done := otel.RecordDuration(ctx, r.histogram, attribute.String("operation", "get_route_rings"))
	defer func() { done(retErr) }()
	...
	query := `
		WITH center AS (
			SELECT AVG(ST_Y(ST_StartPoint(ar.route))) AS center_lat
			FROM desirelines.activity_routes ar
			JOIN desirelines.activities a ON a.id = ar.activity_id
			WHERE a.user_id = $1
		),
		rings AS (
			SELECT unnest($2::int[]) AS ring_m
		)
		SELECT
			r.ring_m,
			ST_AsGeoJSON(
				ST_Translate(
					ST_Buffer(
						ST_SetSRID(ST_MakePoint(0, c.center_lat), 4326)::geography,
						r.ring_m
					)::geometry(Polygon, 4326),
					0,
					-c.center_lat
				)
			)::jsonb -> 'coordinates' -> 0 AS ring_coords
		FROM rings r, center c
		ORDER BY r.ring_m
	`

	rows, retErr := r.db.Query(ctx, query, userID, ringMeters)
	if retErr != nil {
		return nil, fmt.Errorf("query route rings: %w", retErr)
	}
	defer rows.Close()

	rings = make([]repository.RouteRing, 0, len(ringMeters))
	for rows.Next() {
		var ring repository.RouteRing
		var coordsJSON []byte

		if retErr = rows.Scan(&ring.RadiusMeters, &coordsJSON); retErr != nil {
			return nil, fmt.Errorf("scan route ring row: %w", retErr)
		}

		if retErr = json.Unmarshal(coordsJSON, &ring.Coords); retErr != nil {
			return nil, fmt.Errorf("unmarshal ring coords for radius %d: %w", ring.RadiusMeters, retErr)
		}
	...
	if retErr = rows.Err(); retErr != nil {
		return nil, fmt.Errorf("iterate route ring rows: %w", retErr)
	}
