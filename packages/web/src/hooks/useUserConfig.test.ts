    ...
    it("should work with 'annotations' configType and year parameter", async () => {
      ...
      vi.mocked(onSnapshot).mockImplementation((_doc, callback: any) => {
        setTimeout(() => {
          callback({
            exists: () => true,
            data: () => ({ schemaVersion: "2.1", annotations: { "2025": mockAnnotations } }),
          });
        }, 0);
        return vi.fn();
      });
    ...
    it("should work with 'preferences' configType (no year parameter)", async () => {
      ...
      vi.mocked(onSnapshot).mockImplementation((_doc, callback: any) => {
        setTimeout(() => {
          callback({
            exists: () => true,
            data: () => ({ schemaVersion: "2.1", preferences: mockPreferences }),
          });
        }, 0);
        return vi.fn();
      });
    ...
    it("should use default value when no data exists in Firestore", async () => {
      ...
      vi.mocked(onSnapshot).mockImplementation((_doc, callback: any) => {
        setTimeout(() => {
          callback({
            exists: () => true,
            data: () => ({ schemaVersion: "2.1", goals: {} }), // No data for year 2025
          });
        }, 0);
        return vi.fn();
      });
    ...
    it("should call updateConfigSection with correct parameters for preferences", async () => {
      ...
      vi.mocked(onSnapshot).mockImplementation((_doc, callback: any) => {
        setTimeout(() => {
          callback({
            exists: () => true,
            data: () => ({ schemaVersion: "2.1", preferences: { theme: "light" } }),
          });
        }, 0);
        return vi.fn();
      });
    ...
    it("should update annotations section with year", async () => {
      ...
      vi.mocked(onSnapshot).mockImplementation((_doc, callback: any) => {
        setTimeout(() => {
          callback({
            exists: () => true,
            data: () => ({ schemaVersion: "2.1", annotations: {} }),
          });
        }, 0);
        return vi.fn();
      });
