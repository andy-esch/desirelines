...
export const UserConfig: MessageFns<UserConfig> = {
  encode(message: UserConfig, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    globalThis.Object.entries(message.goals).forEach(([key, value]: [string, SportGoalsForYear]) => {
      UserConfig_GoalsEntry.encode({ key: key as any, value }, writer.uint32(82).fork()).join();
    });
    globalThis.Object.entries(message.annotations).forEach(([key, value]: [string, AnnotationsForYear]) => {
...
export const SportGoalsForYear: MessageFns<SportGoalsForYear> = {
  encode(message: SportGoalsForYear, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    globalThis.Object.entries(message.sports).forEach(([key, value]: [string, GoalsForYear]) => {
